...
Image::Image(
	...
	: imageLogicalDevice(logicalDevice), imagePhysicalDevice(physicalDevice), imageSwapchain(swapchain), imageBufferManager(bufferManager) {
...
void Image::createTextureImage() {
	...
	std::shared_ptr<Buffer> stagingBuf = imageBufferManager->getBuffer("texImage_staging");
	...
	copyBufferToImage(stagingBuf->getHandle());
...
void Image::createDepthImage() {
	...
	uint32_t width = imageSwapchain->getSwapchainExtent().width;
	uint32_t height = imageSwapchain->getSwapchainExtent().height;
...
void Image::transitionImageLayout(VkImageLayout newLayout) {
	...
	VkCommandBuffer commandBuffer = imageBufferManager->beginOneTimeCommands();
	...
	imageBufferManager->endOneTimeCommands(commandBuffer);
